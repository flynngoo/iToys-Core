...
import com.itoys.android.core.network.upload.Category
import com.itoys.android.core.network.upload.IUploadRepository
import com.itoys.android.core.network.upload.TokenType
import com.itoys.android.core.network.upload.Upload
...
class QiNiuRepository(
                ...
                if (info.isOK) {
                    ...
                    success?.invoke(
                        (tokenType == TokenType.Public).then(
                            "${NetworkInitialization.requireResourcesHostUrl()}$key", key
                        )
                    )
                } else {
    ...
    override suspend fun syncUploadFile(
        ...
        return if (info.isOK) {
            logcat { "上传成功 -> $info" }
            (tokenType == TokenType.Public).then("${NetworkInitialization.requireResourcesHostUrl()}$uploadKey", uploadKey)
        } else {
