...
object BankCheckUtil {
    ...
    fun checkBankCard(cardId: String): Boolean {
        ...
        return (bit == 'N').then({ false }, { cardId[cardId.length - 1] == bit })
    }
    ...
    fun getBankCardCheckCode(nonCheckCodeCardId: String?): Char {
        if (nonCheckCodeCardId.isBlank() || nonCheckCodeCardId?.matches("\\d+".toRegex()) == false) {
            // 如果传的不是数据返回N
            return 'N'
        }

        val chs = nonCheckCodeCardId.trimString().toCharArray()
        var luhmSum = 0
        var i = chs.size - 1
        var j = 0
        while (i >= 0) {
    ...
    fun getNameOfBank(cardBin: String): String {
        ...
        // 通过银行卡的前6位确定
        val cardbin = cardBin.substring(0, 6)
        var index = -1
